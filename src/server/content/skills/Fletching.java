...
public class Fletching extends SkillHandler {
	...
	private static void showInterfaceFletching(Client c, int[] items, int type) {
		...
		c.getPA().sendFrame126("What would you like to fletch?", 8879);
		...
		c.getPA().sendFrame126(""+c.getItems().getItemName(items[0])+"", 8889);
		c.getPA().sendFrame126(""+c.getItems().getItemName(items[1])+"", 8893);
