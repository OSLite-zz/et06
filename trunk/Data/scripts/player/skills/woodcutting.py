...
def doCut(player, name, objID, x, y):
	...
	if name == "TREE":
		...
		if level >= 75:
			player.sendMessage("You swing your axe at the tree...")
			player.setAnimation(867)
			TaskManager.registerClientTask(TaskFactory.getDelayedTask("callback_woodcutting_MAGIC", player, objID, x, y), random.randrange(6, 18))
		else:
			player.sendMessage("You need a woodcutting level of 75 to cut this tree.")
...
def callback_woodcutting_MAGIC(player, id, x, y):
	...
	if Misc.random(6) == 1:
		...
		doAxeCheck(player, "MAGIC", id, x, y)
