...
public class JourneyShip {
		...
		**/
		...
		client.setCanWalk(false);
			...
			//@Override
			public void execute(EventContainer container) {
				//client.getPA().spellTeleport(client.getAbsX, client.getAbsY, 0);
				container.stop();
			}
			...
			public void stop() {
				...
				client.setCanWalk(true);
				client.getPA().removeAllWindows();
			client.getPA().sendFrame126("Please talk with Infexis Guide to begin your adventures.", 357);
			client.getPA().sendFrame164(356);
			//client.getPA().createArrow(1, Server.getNpcManager().TutorId);
			}
